...
# Portuguese descriptions for noctalia commands
NOCTALIA_DESCRIPTIONS = {
    "panel-toggle launcher": "Abrir launcher",
    "panel-toggle control-center": "Abrir centro de controle",
    "panel-toggle clipboard": "Abrir clipboard",
    "panel-toggle control-center notifications": "Abrir notificações",
    "panel-toggle session": "Abrir menu de sessão",
    "settings-toggle": "Abrir configurações",
    "nightlight-toggle": "Toggle nightlight",
    "caffeine-toggle": "Toggle caffeine",
    "wallpaper-random": "Wallpaper aleatório",
    "theme-mode-toggle": "Toggle tema claro/escuro",
    "session lock": "Bloquear tela",
    "volume-up": "Aumentar volume",
    "volume-down": "Diminuir volume",
    "volume-mute": "Silenciar volume",
    "mic-mute": "Silenciar microfone",
    "brightness-up": "Aumentar brilho",
    "brightness-down": "Diminuir brilho",
    "media toggle": "Play/Pausar",
    "media next": "Próxima faixa",
    "media previous": "Faixa anterior",
    "media stop": "Parar reprodução",
    "screenshot-fullscreen": "Captura de tela fullscreen",
    "screenshot-region": "Captura de tela região",
    "screenshot-fullscreen pick": "Captura de tela com seletor",
}
...
def _generate_description(dispatch: str) -> str:
    ...
    if m:
        cmd = m.group(1)
        # Check for noctalia commands first
        noctalia_match = re.match(r'noctalia msg (.+)', cmd)
        if noctalia_match:
            noctalia_cmd = noctalia_match.group(1)
            return NOCTALIA_DESCRIPTIONS.get(noctalia_cmd, f"Executar {noctalia_cmd}")
...
def parse_keybinds(path: Path) -> list[tuple[str, str, str, str]]:
    ...
    for m in BIND_START_RE.finditer(text):
        ...
        # Check for @desc annotation — search forward from bind line
        # (for multi-line function bindings, @desc may be after end))
        bind_line = text[:m.start()].count("\n")
        desc = None
        for lookahead in range(1, 10):
            if bind_line + lookahead >= len(lines):
                break
            dm = DESC_ANNOTATION_RE.search(lines[bind_line + lookahead])
            if dm:
...
def render(binds: list[tuple[str, str, str, str]]) -> str:
    ...
    for key, desc, action, group in binds:
        ...
        esc_group = (group or "Misc").replace('"', '\\"')
        out.append(f'  {{ "{key}", "{desc}", "{esc_action}", "{esc_group}" }},\n')
    out.append("}\n")
    return "".join(out)
